...
function buildNotification(eventType: PushEventType, nickname: string, entryId: string, siteUrl: string, month = '', commentId = '') {
  const url = new URL(siteUrl);
  if (entryId) url.searchParams.set('entry', entryId);
  if (eventType === 'comment_created' && commentId) url.searchParams.set('comment', commentId);
...
Deno.serve(async (request) => {
  ...
  const { eventType, actorMemberId, entryId, commentId, month } = body;
  ...
  const notification = buildNotification(eventType, actor.nickname, entry?.id || '', siteUrl, month, commentId);
