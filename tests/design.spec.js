...
const checks = [
  ...
  ['home month paging uses native scroll snap instead of JS drag', app.includes('const monthViewportRef = useRef(null)') && app.includes('function settleMonthScroll()') && app.includes('viewport.scrollLeft = getMonthPageWidth(viewport)') && !app.includes('drag="x"') && !app.includes('useAnimationControls') && css.includes('scroll-snap-type: x mandatory') && css.includes('scroll-snap-align: center')],
  ['home mobile scroll axes are separated by native axis locking', css.includes('touch-action: pan-x pan-y') && css.includes('overflow-x: auto') && css.includes('overflow-y: hidden') && css.includes('.week-list') && css.includes('overflow-y: auto')],
  ['home month carousel pages before changing month', app.includes('scrollDelta <= -monthSwipeThreshold') && app.includes('scrollDelta >= monthSwipeThreshold && canGoNextMonth') && app.includes('onChangeMonth(-1)') && app.includes('onChangeMonth(1)')],
  ['home month carousel resyncs to device width', app.includes('function getMonthPageWidth(viewport)') && app.includes('viewport?.clientWidth || screenPushDistance') && app.includes('resetMonthScroll(viewport)') && app.includes('}, [monthDate, screenPushDistance]);') && css.includes('width: max-content')],
  ['home month carousel blocks future month scroll', app.includes('function isFutureMonth(monthStart)') && app.includes('const canGoNextMonth = !isFutureMonth(addMonths(monthDate, 1))') && app.includes("scrollDelta >= monthSwipeThreshold && !canGoNextMonth") && app.includes("viewport.scrollTo({ left: monthPageWidth, behavior: 'smooth' })")],
  ...
  ['home month pages own vertical scroll containers', app.includes('function HomeMonthPage({ weeks, onSelectWeek })') && app.includes('<div className="home-month-viewport" ref={monthViewportRef} onScroll={handleMonthScroll}>') && app.includes('<div className="home-month-track">') && app.includes('<div className="week-list">')],
