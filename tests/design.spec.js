...
const checks = [
  ...
  ['home month carousel pages before changing month', app.includes('pageIndex === 0') && app.includes('pageIndex >= 2 && canGoNextMonth') && app.includes('onChangeMonth(-1)') && app.includes('onChangeMonth(1)')],
  ['home month carousel resyncs to device width', app.includes('viewport.scrollLeft = screenPushDistance') && app.includes('}, [monthDate, screenPushDistance]);') && css.includes('width: max-content')],
  ['home month carousel blocks future month scroll', app.includes('function isFutureMonth(monthStart)') && app.includes('const canGoNextMonth = !isFutureMonth(addMonths(monthDate, 1))') && app.includes("pageIndex >= 2 && !canGoNextMonth") && app.includes("viewport.scrollTo({ left: screenPushDistance, behavior: 'smooth' })")],
