...
홈의 주차별 문구는 Supabase Edge Function `generate-weekly-summaries`가 OpenAI API로 만든 짧은 AI 하이라이트 요약을 보여줍니다. 톤은 귀엽지만 오글거리지 않고, 살짝 웃긴 한 줄 요약입니다. 요약은 브라우저에 캐시되며 일기 본문, 장소, 사진/댓글/좋아요 수가 바뀐 주만 다시 생성됩니다.
